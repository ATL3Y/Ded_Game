...
namespace FluffyUnderware.CurvyEditor
{
    ...
    public class CurvySplineSegmentEditor : CurvyEditorBase<CurvySplineSegment>
    {
        ...
        void userValueElementGUI(Rect rect, int index, bool isActive, bool isFocused)
        {
            var prop = serializedObject.FindProperty(string.Format("{0}.Array.data[{1}]", new object[] { "m_UserValues", index }));
            if (prop != null)
                VectorExPropertyDrawer.DoGUI(rect, prop, new DTPropertyAttribute(),new GUIContent(index.ToString()),DevTools.AttributeOptionsFlags.Full);
        }
        ...
        void userValueAdd(ReorderableList list)
        {
            Target.Spline.InsertUserValueAt(list.index);
            list.elementHeight = EditorGUIUtility.wideMode ? 21 : 42;
            EditorUtility.SetDirty(Target.Spline);
        }

        void userValueDelete(ReorderableList list)
        {
            Target.Spline.RemoveUserValueAt(list.index);
            list.elementHeight = (EditorGUIUtility.wideMode || list.count==1) ? 21 : 42;
            EditorUtility.SetDirty(Target.Spline);
        }
